...
function MagnifierZoom({
  ...
  let invalidVertical = zoomImageDimensions.height <= zoomContainerDimensions.height;
  let invalidHorizontal = zoomImageDimensions.width <= zoomContainerDimensions.width;
  ...
  return (
    ...
    >
      <Image
        ref={zoomImageRef}
        style={{
          ...styles.getLargeImageStyle(invalidHorizontal ? 0 : position.x, invalidVertical ? 0 : position.y, true)
        }}
        src={imageSrc}
        alt={imageAlt}
        onImageLoad={(e) => {
